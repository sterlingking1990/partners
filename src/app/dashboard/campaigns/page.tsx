    ...
    try {
      ...
      const enriched = await Promise.all(
        (rawCampaigns || []).map(async (camp: any) => {
          ...
          // If no items in status_media, check if status_posts has a direct media_url
    ...
    <div className="fixed inset-0 z-[200] bg-black flex items-center justify-center animate-in fade-in duration-300">
       ...
       <div className="absolute top-8 left-0 right-0 px-6 flex items-center justify-between z-[210]">
          <div className="flex items-center gap-3">
             <div className="h-10 w-10 rounded-xl overflow-hidden border border-white/20">
                <img src={currentStatus.brand.avatar} className="h-full w-full object-cover" />
             </div>
             <div>
       ...
       <div className="relative w-full h-full flex items-center justify-center select-none">
          {currentMedia.media_type === 'video' ? (
            ...
            <img src={currentMedia.media_url} className="max-h-full max-w-full object-contain" />
          )}
...
function CampaignCard({ campaign, completed, onClick }: { campaign: any, completed?: boolean, onClick: () => void }) {
  const getTypeColor = (type: string) => {
    switch(type) {
      ...
      case 'challenge': return 'bg-purple-50 text-purple-600'
      case 'survey': return 'bg-blue-50 text-blue-600'
    ...
    <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-xl hover:border-brand/30 transition-all group flex flex-col h-full relative overflow-hidden">
       ...
       <div className="flex items-center gap-4 mb-6 relative z-10">
          <div className="h-12 w-12 rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
             <img src={campaign.brand.avatar || `https://ui-avatars.com/api/?name=${campaign.brand.name}&background=random`} className="h-full w-full object-cover" />
          </div>
          <div>
